...
forceDataMap :: M.Map a b -> ()
forceDataMap m = if M.size m > 0 then snd (M.findMin m) `seq` () else ()

keep :: Moore (Either ((a,M.Map a Int), Int) Int) (Report a)
keep = let
  make  r = Moore r (make . ($ r) . either bset (set nulls))
  bset ((x,m),i) = set forceDict (readOne m) . set hits i . set maxout (Just x)
